...
func (cr *RevenueHandler) FindByFilters(ctx *gin.Context) {
	...
	filters["value"] = revenuesFilter.Value
...
func (cr *RevenueHandler) Delete(ctx *gin.Context) {
	...
	if revenue.UserID != userId {
		...
		log.NewLogger().Error(fmt.Errorf("a receita com id %d não está relacionado com o usuário logado com id %d", revenue.UserID, userId))
		return
	}
